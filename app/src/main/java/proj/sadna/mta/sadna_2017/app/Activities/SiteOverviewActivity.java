...
public class SiteOverviewActivity extends AppCompatActivity
{
    ...
    private void initSite()
    {
        if (getIntent().hasExtra("id"))
        {
            ...
            site = SiteModel.findById(SiteModel.class, id);
        } else
        {
    ...
    private void initView()
    {
        ...
        addBtn.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View view)
            {
                ...
                {
                    long id = getIntent().getLongExtra("path_id", 1);
                    if (PathModel.findById(PathModel.class, id).getIds().split(",").length < 5)
                    {
                        PathModel.addSite(id, site.getId());
                        Toast.makeText(SiteOverviewActivity.this, "Added", Toast.LENGTH_LONG).show();

                        setResult(123);
                        finish();
                    } else
                    {
                        Toast.makeText(SiteOverviewActivity.this, "5 Sites at top!", Toast.LENGTH_LONG).show();
                        finish();
                    }
